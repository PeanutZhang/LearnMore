...
public class ImageUtil {
    ...
    public static final Intent takeBigPicture(Context context) {
        ...
        intent.putExtra(MediaStore.EXTRA_OUTPUT, newPictureUri(getNewPhotoPath(context)));
        return intent;
    }

    public static final String getDirPath(Context mcontext) {
      return   mcontext.getCacheDir().getPath()+"/UploadImage";
      //  return Environment.getExternalStorageDirectory().getPath() + "/UploadImage";
    }

    private static final String getNewPhotoPath(Context mcontext) {
        return getDirPath(mcontext) + "/" + System.currentTimeMillis() + ".jpg";
    }
            ...
            if (sourceIntent != null) {
                ...
                if (!TextUtils.isEmpty(picPath)) {
                    ...
                    if (!file.exists() || !file.isFile()) {
                        Log.e(TAG, String.format("retrievePath file not found from sourceIntent path:%s", picPath));
                    }
                }
            }
            return picPath;
